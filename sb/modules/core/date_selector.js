...
function CalendarPopup() {
	...
	{
		c.returnFunctionCall = arguments[1];
		c.returnFunctionCallParams = arguments[2];
	}
	else
	{
		c.returnFunctionCall = '';
		c.returnFunctionCallParams = '';
	}
...
function CP_tmpReturnFunction(y,m,d,call_back_function,call_back_params) { 
	if (window.CP_targetInput!=null) {
		var dt = new Date(y,m-1,d,0,0,0);
		window.CP_targetInput.value = formatDate(dt,window.CP_dateFormat);

		if((String(call_back_function) != 'undefined') && (String(call_back_function) != ''))
		{
			if((String(call_back_params) != 'undefined')  && (String(call_back_params) != ''))
			{
				var function_to_call = call_back_function + '(\'' + formatDate(dt,window.CP_dateFormat) + '\',' + call_back_params + ')';
			}
			else
			{
				var function_to_call = call_back_function + '(\'' + formatDate(dt,window.CP_dateFormat) + '\')';				
			}
...
function CP_setReturnFunctionCall(name) { this.returnFunctionCall = name; }
function CP_setReturnFunctionCallParams(name) { this.returnFunctionCallParams = name; }
...
function CP_getCalendar() {
	...
	if (this.displayType=="date" || this.displayType=="week-end") {
		...
		for (var row=1; row<=6; row++) {
			...
			for (var col=1; col<=7; col++) {
				...
				if (disabled || this.disabledWeekDays[col-1]) {
					...
					result += '	<TD CLASS="'+this.cssPrefix+dateClass+'"><A HREF="javascript:'+windowref+this.returnFunction+'('+selected_year+','+selected_month+','+selected_date+',\''+this.returnFunctionCall+'\',\''+this.returnFunctionCallParams+'\');'+windowref+'CP_hideCalendar(\''+this.index+'\');" CLASS="'+this.cssPrefix+dateClass+'">'+display_date+'</A></TD>\n';
					}
				display_date++;
		...
		if (disabled || this.disabledWeekDays[current_weekday+1]) {
			...
			result += '		<A CLASS="'+this.cssPrefix+'cpTodayText" HREF="javascript:'+windowref+this.returnFunction+'(\''+now.getFullYear()+'\',\''+(now.getMonth()+1)+'\',\''+now.getDate()+'\',\''+this.returnFunctionCall+'\',\''+this.returnFunctionCallParams+'\');'+windowref+'CP_hideCalendar(\''+this.index+'\');">'+this.todayText+'</A>\n';
			}
		result += '		<BR>\n';
