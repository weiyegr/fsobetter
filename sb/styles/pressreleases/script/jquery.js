...
var tb_pathToImage = "/sb/styles/standard/images/loadingAnimation.gif";
...
function get_url_key_value(url, key)
{
...
function tb_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
	...
	try {
		...
		var parent;
		if(url.indexOf('parent'))
		{
			parent = get_url_key_value(url, 'parent');
			//alert(parent);
			$("#TB_window").addClass(parent);
		}
	   ...
	   var urlType = baseURL.toLowerCase().match(urlString);
		...
		if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
			...
			TB_WIDTH = (params['width']) || 630; //defaults to 630 if no paramaters were added to URL
			TB_HEIGHT = (params['height']) || 440; //defaults to 440 if no paramaters were added to URL
			...
			if(url.indexOf('TB_iframe') != -1){// either iframe or ajax window		
					...
					if(params['modal'] != "true"){//iframe no modal
						$("#TB_window").append("<div id='TB_title'><div id='TB_ajaxWindowTitle'></div></div><iframe frameborder='0' hspace='0' src='"+urlNoQuery[0]+"' id='TB_iframeContent' name='TB_iframeContent"+Math.round(Math.random()*1000)+"' onload='tb_showIframe()' style='width:"+(ajaxContentW)+"px;height:"+(ajaxContentH)+"px;' > </iframe>");
					}else{//iframe modal
					$("#TB_overlay").unbind();
						$("#TB_window").append("<div id='TB_title'><div id='TB_ajaxWindowTitle'></div></div><iframe frameborder='0' hspace='0' src='"+urlNoQuery[0]+"' id='TB_iframeContent' name='TB_iframeContent"+Math.round(Math.random()*1000)+"' onload='tb_showIframe()' style='width:"+(ajaxContentW)+"px;height:"+(ajaxContentH)+"px;'> </iframe>");
					}
