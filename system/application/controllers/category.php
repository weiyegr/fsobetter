...
class Category extends Controller{
	...
	function index($short='',$searchtxt='0',$offset=0){ 
		...
		$menuL=array();
		while($parentid!=0){
		    $query=$this->db->query("select * from type where id='".$parentid."' order by id desc");
		    $row=$query->row();
			$parentid=$row->parentid;
			$on[$row->short]=$row->title;
			$menuL[$row->short]=$row->title;
		}
		$data['menuL']=array_reverse($menuL);
	...
	function search($short='',$searchtxt='0',$offset=0){ 
		...
		$on=array();
		while($parentid!=0){
		    $query=$this->db->query("select * from type where id='".$parentid."' order by id asc");
		    $row=$query->row();
			$parentid=$row->parentid;
			$on[$row->short]=$row->title;
		}
		...
		$on=array();
			...
			$this->db->query("update archive set hits=hits+1 where id='".$row->id."'");
       ...
       //产品详细
		...
		$this->load->view($pshort.'_detail',$data);
		}
